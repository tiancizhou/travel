...
# ─── 第一阶段：Grok 搜索提炼（OpenAI 兼容协议） ───

POINT_RESEARCH_PROMPT = """请搜索以下点位的相关旅行信息，只做事实提炼，不要分析或推荐。

点位名称：{label}
点位坐标：{lng},{lat}

请给出：
1. 该点位本身的简要介绍（1-2句话，说明它是什么、为什么有名）
2. 周边值得关注的景点或区域（2-4个，列出名称和一句话特点）
3. 信息来源（写明信息来自哪个网站或来源）
...
async def _call_openai(cfg: dict, prompt: str, max_tokens: int = 512) -> str:
    """调用 OpenAI 兼容接口（Grok / 其他中转模型）。"""
    cfg = dict(cfg)  # snapshot
    async with httpx.AsyncClient() as client:
        resp = await client.post(
...
async def _call_anthropic(cfg: dict, prompt: str, max_tokens: int = 512) -> str:
    """调用 Anthropic 兼容接口（GLM）。"""
    cfg = dict(cfg)  # snapshot
    async with httpx.AsyncClient() as client:
        resp = await client.post(
            f"{cfg['base_url']}/v1/messages",
...
async def generate_point_research(
    label: str,
    lng: float,
    lat: float,
) -> str | None:
    """用 Grok 搜索点位信息。失败时返回 None（调用方降级）。"""
    prompt = POINT_RESEARCH_PROMPT.format(label=label, lng=lng, lat=lat)
    try:
        return await _call_openai(_point_cfg, prompt, max_tokens=400)
    except Exception:
        return None


# ─── 第二阶段：GLM 分析总结（Anthropic 兼容协议） ───

POINT_ANALYSIS_WITH_RESEARCH_PROMPT = """你是一位本地旅行顾问，请综合搜索资料和周边景点数据，输出结构化点位分析。

点位名称：{label}
点位坐标：{lng},{lat}

搜索资料（来自互联网搜索，可能有不准确之处）：
{research}

周边景点（来自地图数据）：
{landmarks}

严格按以下格式输出，不要加任何额外文字或解释：

VERDICT: 值得停留 / 可顺路经过 / 不建议特意前往（三选一）
REASON: 一句话理由（20字内）
LANDMARKS:
- 景点名称｜一句话特点
- 景点名称｜一句话特点
TRANSIT: 一句话交通说明（如无可靠信息则写"暂无交通数据"）

要求：
1. 综合两份数据，以地图景点数据为主、搜索资料为辅。
2. 如果搜索资料与地图数据冲突，以地图数据为准。
3. LANDMARKS 列出 2-3 个周边代表景点。
4. TRANSIT 写最近地铁站或公交站的步行距离，不要编造。
5. 总字数控制在 80 到 120 字。"""

POINT_ANALYSIS_FALLBACK_PROMPT = """你是一位本地旅行顾问，请针对一个具体点位输出结构化景点分析。
...
要求：
1. 如果周边有代表性景点，列出 2-3 个并简述特点。
2. TRANSIT 写最近地铁站或公交站的步行距离，不要编造。
3. 总字数控制在 80 到 120 字。"""
...
async def generate_point_analysis(
    ...
    if research:
        prompt = POINT_ANALYSIS_WITH_RESEARCH_PROMPT.format(
            label=label,
            lng=lng,
            lat=lat,
            research=research,
        ...
        prompt = POINT_ANALYSIS_FALLBACK_PROMPT.format(
            label=label,
            lng=lng,
            lat=lat,
            landmarks=landmark_lines,
    ...
    raw = await _call_anthropic(_guide_cfg, prompt, max_tokens=256)
    return parse_point_analysis(raw)
