...
POINT_ANALYSIS_WITH_RESEARCH_PROMPT = """你是一位本地旅行顾问，请综合以下搜索资料和周边景点数据，生成简洁可信的点位分析。
...
要求：
1. 综合两份数据写分析，以地图景点数据为主、搜索资料为辅。
2. 如果搜索资料与地图数据冲突，以地图数据为准。
3. 说明这个点位是否值得停留及原因。
4. 列出2-4个周边代表景点并简述理由。
5. 给出建议停留时长。
6. 不要编造交通线路、营业时间或门票信息。
7. 控制在180到260字。"""

POINT_ANALYSIS_FALLBACK_PROMPT = """你是一位本地旅行顾问，请只针对一个具体点位做简洁、可信的景点分析。
...
async def generate_point_analysis(
    ...
    return await _call_anthropic(_guide_cfg, prompt)
